...
interface StrokeTraceState {
  started: boolean
  nextCheckpoint: number   // Highest sequentially-reached path index
  pointCount: number
...
export function TracingCanvas({ char, onComplete, onStrokeComplete, onStrokeFailed, onDemoPlay }: Props) {
  ...
  const drawGuide = useCallback(() => {
    ...
    if (showGuideZones && currentStroke < paths.length) {
      ...
      if (stroke.length >= 2) {
        ...
        for (const pt of stroke) {
          ctx.beginPath()
          ctx.arc(pt[0], pt[1], PATH_RADIUS, 0, Math.PI * 2)
          ctx.fill()
        }
        ctx.restore()
        ...
        ctx.fillStyle = '#4caf50'
        ctx.beginPath()
        ctx.arc(stroke[0][0], stroke[0][1], START_RADIUS, 0, Math.PI * 2)
        ctx.fill()
        ctx.restore()
        ...
        ctx.beginPath()
        ctx.arc(last[0], last[1], END_RADIUS, 0, Math.PI * 2)
        ctx.fill()
        ctx.restore()
      }
    }
    ...
    paths.forEach((stroke, idx) => {
      ...
      const startX = stroke[0][0], startY = stroke[0][1]
      const circleR = 14
      ctx.save()
      if (idx < currentStroke) {
        ctx.globalAlpha = 0.4; ctx.fillStyle = '#4caf50'
      } else if (idx === currentStroke) {
        ctx.globalAlpha = hintPulse ? 1.0 : 0.9; ctx.fillStyle = '#e85d3a'
        if (hintPulse) {
          ctx.beginPath(); ctx.arc(startX, startY, circleR + 8, 0, Math.PI * 2)
          ctx.strokeStyle = 'rgba(232, 93, 58, 0.5)'; ctx.lineWidth = 3; ctx.stroke()
        }
      } else {
        ctx.globalAlpha = 0.3; ctx.fillStyle = '#8b7355'
      }
      ctx.beginPath(); ctx.arc(startX, startY, circleR, 0, Math.PI * 2); ctx.fill()
      ctx.fillStyle = 'white'
    ...
    if (currentStroke < paths.length) {
      ...
      if (stroke.length >= 2) {
        ...
        ctx.setLineDash([6, 6]); ctx.lineCap = 'round'; ctx.lineJoin = 'round'
        ...
        if (stroke.length === 2) {
          ...
          for (let i = 0; i < stroke.length - 1; i++) {
            ...
            const p1 = stroke[i], p2 = stroke[i + 1]
  ...
  useEffect(() => {
    ...
    guide.width = CANVAS_RES; guide.height = CANVAS_RES
    draw.width = CANVAS_RES; draw.height = CANVAS_RES
    completedStrokes.current = new Set()
    setCurrentStroke(0); setExtractedPaths(null); extractedPathsRef.current = null
    resetTraceState(); setStrokeScore(null); setStrokeFailed(false)
  ...
  const playDemo = useCallback(() => {
    ...
    setCurrentStroke(0); setStrokeScore(null); setStrokeFailed(false)
    ...
    const animate = () => {
      if (strokeIdx >= paths.length) {
        ...
        demoTimeoutRef.current = setTimeout(() => {
          if (drawCtx) drawCtx.clearRect(0, 0, CANVAS_RES, CANVAS_RES)
          completedStrokes.current = new Set(); resetTraceState(); setCurrentStroke(0)
        }, 800); return
      }
      const stroke = paths[strokeIdx]; if (!drawCtx) return
      if (pointIdx === 0) {
        drawCtx.beginPath(); drawCtx.lineCap = 'round'; drawCtx.lineJoin = 'round'
        drawCtx.lineWidth = PEN_WIDTH; drawCtx.strokeStyle = `rgba(45, 32, 22, ${TRACED_OPACITY})`
        drawCtx.moveTo(stroke[0][0], stroke[0][1])
      }
      if (pointIdx < stroke.length) {
        drawCtx.lineTo(stroke[pointIdx][0], stroke[pointIdx][1]); drawCtx.stroke()
        drawCtx.beginPath(); drawCtx.moveTo(stroke[pointIdx][0], stroke[pointIdx][1])
        pointIdx++; demoAnimRef.current = requestAnimationFrame(animate)
      } else {
        completedStrokes.current.add(strokeIdx); setCurrentStroke(strokeIdx + 1)
  ...
  const triggerHint = useCallback(() => {
    setHintPulse(true); setTimeout(() => setHintPulse(false), 800)
  }, [])
  ...
  const handleStart = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    setStrokeFailed(false); setStrokeScore(null)
    ...
    const ctx = drawCanvasRef.current?.getContext('2d')
    if (ctx) {
      ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.lineWidth = PEN_WIDTH
  ...
  const handleMove = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    // Sequential checkpoint advancement with max jump limit
    const maxJump = Math.max(1, Math.floor(stroke.length * MAX_CHECKPOINT_JUMP_RATIO))
    const current = traceState.current.nextCheckpoint
    // Only advance if nearestIdx is ahead but not too far ahead
    if (nearestIdx >= current && nearestIdx <= current + maxJump) {
      traceState.current.nextCheckpoint = nearestIdx + 1
    }
    // If nearestIdx is slightly behind (tolerance of 10% of stroke), keep current
    // If nearestIdx jumped way ahead, DON'T update (prevents skipping)
  }
  ...
  const handleEnd = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    if (!ts.started || ts.pointCount < MIN_SAMPLE_POINTS) {
      resetTraceState(); return
    }

    // All ratio-based scoring
    const avgDist = ts.totalDistance / ts.pointCount
    const coverage = ts.nextCheckpoint / stroke.length
    const farStreakRatio = ts.maxConsecutiveFar / ts.pointCount

    // Check if finger ended near the stroke's end point
    const lastTouch = drawCanvasRef.current?.getContext('2d') ? lastPos.current : null
    const endPoint = stroke[stroke.length - 1]
    // Use the last recorded position before mouseup (we need to check it before clearing)
    // Since lastPos is already null, we check coverage instead + endpoint distance via last checkpoint
    const lastCheckpointIdx = Math.min(ts.nextCheckpoint, stroke.length - 1)
    const endpointCovered = lastCheckpointIdx >= stroke.length * MIN_COVERAGE

    // Pass conditions (all ratio-based):
    const passed =
      coverage >= MIN_COVERAGE &&           // Covered 90%+ of path sequentially
      endpointCovered &&                     // Reached near end
      avgDist <= SCORE_THRESHOLD &&          // Average distance within threshold
      farStreakRatio < MAX_FAR_STREAK_RATIO  // No excessive wandering

    setStrokeScore(Math.round(avgDist))
    ...
    if (passed) {
      ...
      else { setCurrentStroke(next); setStrokeScore(null) }
    } else {
      setStrokeFailed(true); triggerHint(); onStrokeFailed?.()
      setTimeout(() => {
        clearDrawCanvas(); setStrokeFailed(false); setStrokeScore(null); resetTraceState()
      }, 1000)
    }
  }
  ...
  return (
    <div className="canvas-container" ref={containerRef}>
      ...
      <canvas
        ref={drawCanvasRef} style={{ zIndex: 2 }}
      ...
      />
      <div className="canvas-top-buttons">
        <button
          className={`demo-btn ${isPlayingDemo ? 'playing' : ''}`}
          onClick={playDemo} disabled={isPlayingDemo}
        >
          {isPlayingDemo ? '再生中…' : '▶ お手本'}
        </button>
        <button
          className={`guide-btn ${showGuideZones ? 'active' : ''}`}
          onClick={() => setShowGuideZones(v => !v)}
        >
      ...
      <div className="stroke-order-dots" style={{ position: 'absolute', bottom: 12, left: 0, right: 0, zIndex: 3 }}>
        {char.strokes.map((_, idx) => (
          <div key={idx} className={`stroke-dot${
            completedStrokes.current.has(idx) ? ' completed' : idx === currentStroke ? ' current' : ''
          }`} />
        ))}
      </div>
    </div>
